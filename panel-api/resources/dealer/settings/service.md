...
* `display_model_features_link` - boolean. Shows a link to gdemoi.ru in model info if `true` (UI option).
...
* `default_user_time_zone` - string. [Time zone ID](../../timezone.md) for new users created via [user/upload](../../user/index.md#upload). This time zone is also selected by default when creating a new user in the GdeMoi Admin Panel.
...
| display_model_features_link | When `true` shows in model info link to gdemoi.ru (UI option).                  | boolean                                                                          |
