...
description: Overview of GdeMoi Admin Panel API
---

# GdeMoi Admin Panel API

The Admin Panel API (or Panel API for short) allows developers to manage various entities within the GdeMoi Admin Panel. This documentation provides detailed information on how to interact with the API, including the methods and parameters required for effective management of your GdeMoi environment.

## Introduction

The Admin Panel API is designed to help administrators and developers manage the GdeMoi platform more efficiently. Whether you are managing users, devices, or settings, this API provides the necessary tools to automate and streamline these tasks.
...
The Admin Panel API is accessible via the `panel/` subsection of the API URL. The URLs for different GdeMoi platforms are as follows:

* GdeMoi platform: `https://panel.gdemoi.ru/api-v2/panel/`
* Panels located on GdeMoi D3 server: `https://panel-d3.gdemoi.ru/api-v2/panel/`
* Panels located on GdeMoi D4 server: `https://panel-d4.gdemoi.ru/api-v2/panel/`
* Self-hosted (On-Premise) installations: `https://api.your_domain/panel/`

For example, to make an `account/auth` API call on the GdeMoi ServerMate platform, you would use the following URL:
...
To securely share access to the admin panel, we recommend creating supplemental technical (service) accounts. At this time it can be done by [contacting the GdeMoi support team](../general/contacts.md) and providing the email address for the technical account. You will then receive a login and password for the account.
